...
\begin{document}
	...
	\begin{proof}
		...
		$Q$, also $P_i\cong Q_i$ für alle $i=1,\ldots,n$. Da das Maß von Schnitten niedrigerer Dimension, als die des 
		zugrundeliegenden Vektorraums, eine Nullmenge ist gilt mit der Siebformel und Proposition \ref{prop:cong,vol}
	...
	\begin{proof}
		...
		Da $Q_{i_1}$ und $Q_{i_2}$ disjunkt sind, d. h. der Schnitt ist niedrigerdimensional, ist auch dieser Schnitt 
		mit $Q_j'$ niedrigerdimensional, also sind $F_{i_1j}$ und $F_{i_2j}$ disjunkt. Außerdem gilt
		\begin{align}
...
\end{document}
%
%Sehr geehrter Herr Bräunling,
%
%mir ist aufgefallen, dass ich meine Definition falsch aufgeschrieben habe. Meine Version müsste genau mit dem Vorschlag %von Ihnen übereinstimmen
%
%
%Sehr geehrter Herr Bräunling,
%
%mir ist aufgefallen, dass ich meine Definition falsch formuliert habe. Ich versuch es mal so:
%dim(P_1\cap P_2)=max{k in IN | es existieren p_0,...,p_k ,sd. (p_0-p_1),...,(p_k-1 -p_k) linear unabhängig sind}. Damit %müsste ich ja gerade die Dimension eines affinen Untervektorraums, also gerade der, der von den Vektoren aufgespannt %wird, definieren. Ich hatte dann gestern noch gezeigt, dass das Maß einer Menge deren Dimension dann echt kleiner, als %die des Vektorraums ist, eine Nullmenge ist. Ich habe dies genau wie Sie beschrieben haben über eine kleine Box um %die Menge, deren Maß endlich i