...
\begin{document}
	...
	Ergänzungsgleichheit von Polytopen. In diesem Kapitel zeigen, dass 
	...
	\begin{definition}[Simplex]\label{def:simplex}
		Ein Simplex $S$ im $\setR^d$ ist die konvexe Hülle einer Menge von 
		$d+1$ Punkten 
	...
	\begin{remark}[Parameterdarstellung des Simplex]\label{bem:paradarst;simplex}
		Für einen Simplex $S=\langle p;s_1,\ldots s_n\rangle$ stellen wir 
	...
	\begin{proof}
		...
		bewiesen. Betrachten wir die $d-1$-dimensionalen Seiten von 
		$P'$, dann wissen wir nach Induktionsvoraussetzung, dass diese 
		sich in endliche viele $d-1$-dimensionale Simplizes $S_i'$ zerlegen lassen. 
		...
		von $P'$, so erhalten wir eine, wegen der Konvexität von $P'$, disjunkte 
	...
	\begin{definition}[Parallelotop]
		...
		haben. Dann ist das Parallelotop $P$ definiert durch 
	...
	\begin{definition}[Translationsgleichheit, translative Zerlegungsgleichheit]
		Wir nennen zwei Polytope $P$ und $Q$ translationsgleich, wenn es 
	...
	\begin{definition}[Simplotop]
		...
		das Simplotop $S$ gerade das Polytop, dessen Elemente $s$ 
	...
	des Polytops $Q$ ist. Also soll es ein Polytop geben, zu dem 
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			$P\sim_{\subset}\alpha W$ gilt. Dieses $\delta$ existiert und 
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			$Q\sim \lambda_2 W$, wobei mit $W$ hier der $d$-dimensionale 
			...
			Da $d-i>0$ und o.B.d.A. $vol(R)>0$ gibt es eine natürliche Zahl $n$, s. d. 
			\begin{align}
			...
			ist $n^i\cdot R$ zerlegungsgleich zu einem Teilpolytop von $P_0$, d. h. es gibt Polytope $C,D\subset P_0$ mit $P+Q=P_0$ und $n^i\cdot R\sim C$, s. d. 
			\begin{align}
	...
	Zerlegungsgleichheit oftmals als intuitivere Variante. Auch sind hier bei 
