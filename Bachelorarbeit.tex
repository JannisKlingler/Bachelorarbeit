...
\begin{document}
	...
	Damit ist der bilineare Operator gefunden. Wir wollen für unser Problem den Spezialfall \linebreak
	...
	von $(r,s)$ und $S^{-1}R$ 
	für die Menge aller \linebreak Äquivalenzklassen. 
	...
	Siehe auch Beispiel 1 \cite{Proofsfromthebook}[Kapitel 10].
	...
	Wir wollen uns nun dem zweiten Teil von Hilberts Problem widmen und zwar 
	der \linebreak
	...
	\begin{proof}
		Sei für die Hinrichtung $P\cong Q$ gegeben. Somit gibt es eine Isometrie 
		$f$, s.~d. $f(P)=Q$. Wir können die Isometrie $f$ durch ein $\alpha\in
		\setR^d$ und eine orthogonale Matrix $U\in O(d)$ darstellen, s.~d. 
		$f(P)=\alpha + UP$ gilt. Betrachte die Abbildung $f_{\lambda}$, die 
		für alle $x\in\setR^d$ durch 
		$f_{\lambda}(x)=\lambda\cdot \alpha +Ux$ definieren. 
		Offensichtlich ist $f_{\lambda}$ eine Isometrie. 
		Dann gilt
		...
		Sei für die Rückrichtung nun $\lambda P\cong \lambda Q$, folglich  
		gibt es eine Isometrie $f$, s.~d. $f(\lambda P)=\lambda Q$. Es gibt 
		also wieder $\alpha\in\setR^d$ und $U\in O(d)$, s.~d. 
		$f(x)=\alpha + Ux$ für alle $x\in\setR^d$ gilt. Betrachte die Abbildung 
	...
	\begin{proof}
		...
		Für die Rückrichtung seien $\lambda P\sim \lambda Q$, dann gibt es 
		endlich viele Polytope $P_1,\ldots P_n$ und $Q_1\ldots Q_n$ mit 
	...
	\begin{definition}[Vervielfachung]
		...
		von $P$ mit 
		einer natürlichen Zahl $n$ ein Polytop $n\cdot P$, 
	...
	Die folgenden Eigenschaften ergeben sich beim Vervielfältigen.
	...
	\begin{remark} \label{bem:mink}
		...
		\begin{enumerate}
			\item Die Minkowski-Addition erbt die Kommutativität und Assoziativität von der Addition, d.~h. 
			es gilt
	...
	Mit $\langle p_i,p_j\rangle$ ist hier das Standardskalarprodukt gemeint. 
	...
	\begin{proof}
		...
		dieser Vektoren ein Vielfaches eines anderen der Vektoren.
	...
	\begin{remark}
		Wir stellen fest, dass sich ein wie oben definiertes Simplotop $S$ 
	...
	\begin{theorem}[Folgerung aus der Kanonischen Simplexzerlegung]\label{thm:kanonsimpl}
		Sei $S$ ein Simplex und \linebreak$0<\lambda<1$ gegeben, dann 
	...
	\begin{definition}[Ergänzungsgleich]
		...
		$P_1,\ldots,P_n$, $Q_1,\ldots,Q_n$ gibt, wobei $P_i\cong Q_i$ für alle $i\in\{1,\ldots,n\}$ gilt, s.~d. die Polytope
		\[P'=P+P_1+\ldots+P_n,\qquad Q'=Q+Q_1+\ldots+Q_n\]
		zerlegungsgleich sind.
	\end{definition}
	
	Alternativ sind zwei Poltope $P$ und $Q$ ergänzungsgleich, falls es 
	zwei zerlegungsgleiche Polytope 
	...
	\begin{proof}
		Seien $P$ und $Q$ ergänzungsgleich, d.~h. es gibt endlich viele Polytope $P_1,\ldots,P_n,\linebreak
	...
	\begin{remark}
		...
		$P\sim R''$ und die Transitivität ist gezeigt.
	\end{remark}
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			in dem Würfel $\beta W$ enthalten und wir finden  
	...
	\newpage
	\begin{proof}(Satz \ref{thm:zerlerg})
		\noindent
		\begin{itemize}
	...
	die Beweise oft leichter zu führen, als bei der Ergänzungsgleichheit. 
	Für Hilberts Problem reicht es jedoch aus, die Beweise für eine 
