...
\begin{document}
	...
	\end{definition}
	 ...
	 Isometrien sind als gerade Bewegungen, das heißt sie setzen sich 
	 gerade aus einer Translation und einer Drehung zusammen. Eine 
	 Isometrie $f:\setR^d\to \setR^d$ hat also die Darstellung 
	 $f(x)=\alpha + Q(x)$ für alle $x\in \setR^d$, wobei $\alpha\in\setR^d$ ein Vektor und $Q\in O(d)$ hier eine orthogonale Matrix ist, also eine 
	 Drehung. Da das Skalarprodukt translationsinvariant ist und 
	...
	\begin{proof}
		Da sich eine Isometrie für alle $x\in\setR$ darstellen lässt als $f(x)=\alpha +Q(x)$ für eine orthogonale Matrix $Q\in O(d)$ und ein 
		$\alpha\in\setR^d$ ergibt sich die Aussage mit der Transformationsformel 
		aus der Analysis.
	\end{proof}
	...
	\begin{theorem}[Simplizialzerlegung von Polytopen]
	...
	\begin{remark}\label{bem:dilvol}
		...
		der Transformationsformel aus der Analysis
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			&\sim n^iR +D+n^i \cdot P \notag\\
			&\overset{\ref{coroll:vervielfältigung}}{\sim} D+n^i\cdot(R+P) \notag\\ 
			...
			%... aus nP sim nQ folgt P sim Q. Folgerung bei Dilatation
