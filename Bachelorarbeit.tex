...
\begin{document}
	...
	Vektorraum, das Polytop also keine inneren Punkte hat. Ein solches Polytop 
	wollen wir uneigentlich nennen. Ein Polytop, das innere Punkte besitzt, 
	nennen wir dementsprechend eigentlich. Wir meinen in dieser Arbeit, falls 
	nicht anders genannt, mit Polytopen immer eigentliche Polytope.
	...
	%... hier eventuell noch alles auf Polytope verallgemeinern
	...
	\begin{remark} \label{bem:mink}
		\begin{itemize}
			...
			\item Weiter gelten auch die Distributivgesetze, also 
			...
			P\times (Q+R)=(P\times Q)+(P\times R). \label{bem:minkowsk;distr}
			\end{align}
	...
	\begin{proof}
		\begin{enumerate}
			...
			\end{align*}
			für alle $j=1,\ldots,n$, wobei $P_{j}\cong P$ und 
			...
			gilt. Es gilt also mit Bemerkung \ref{bem:mink} auch
			...
			wobei $P_{k,0}\in \mathfrak{B}_2^{(d_k)}$ und $P_{k,j}\cong P_k$ 
			...
			die wir mit den Distributivgesetzen \ref{bem:minkowsk;distr} 
			umformen können zu
			...
			\[P_{k,\mu_k}\cong P_k\]
			für alle $\mu_k=1,\ldots,n$ und damit
			\[\bigtimes_{k=1}^i P_{k,\mu_k}=
			P_{1,\mu_1}\times\ldots\times P_{i,\mu_i} \cong 
			P_1\times\ldots P_i =P.\]
