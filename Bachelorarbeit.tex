...
\begin{document}
	...
	\end{definition}
	 ...
	 Translationsgruppe hat als Untergruppe also die gleiche Eigenschaften wie die 
	...
	\begin{proof}
		...
		Punkt $m$ in den beiden Dreiecken gleich ist und somit haben die 
	...
	Wir haben also den Fall in der Ebene abgeschlossen und können in 
	...
	der Knackpunkt.
	Deshalb müssen uns zunächst überlegen, was mit den Kanten eines Polyeders $P$ passiert, wenn wir diesen in zwei 
	Polyeder $P_1$ und $P_2$ zerlegen. 
	\begin{definition}
		Sei $k$ also eine Kante des Polyeders $P$.  Wir wollen jeder Kante $k$ ihre \textsl{Länge} $\ell(k)\in\setR_+$, also den größten 
	...
	\end{definition}
	Sei also $\ell(k)=l$ die Länge der Kante $k$ und $w(k)=\varphi$ der 
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\item Zu \ref{bem:dehn;3}: Wir schneiden nicht durch die Kante $k$ von $P$, also finden wir die Kante mit gleicher Länge und gleichem 
	...
	Im Jahr 1965 bewies der schweizer Mathematiker Jean-Pierre Sydler 
	sogar, dass dies eine notwendige Bedingung ist und 
	es ergibt sich der folgende Satz (siehe dazu auch \cite{Sydler1965}).
